...
一个用于管理 Claude Code 设置快照和模板的 CLI 工具（v0.5.0），让你轻松在不同的 AI 提供商之间切换。
...
4. **测试**：在 `main.rs` 的 `#[cfg(test)]` 模块中添加测试
